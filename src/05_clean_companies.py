...
# Optional hand-curated mapping of company → NACE sector.
# Generated by scripts/research_company_sectors.py and filled in via web research.
# If absent, the 'sector' column is added as null for all rows (graceful degradation).
# IMPORTANT: the join key (company_name_clean) must stay in sync with clean_company_name().
# Regenerate company_sector_map.csv after any change to clean_company_name() logic.
...
def build_company_permits() -> pd.DataFrame:
    ...
    # ── Sector enrichment ─────────────────────────────────────────────────────
    # Left-join the hand-curated sector map (if present) on company_name_clean.
    # The join key must exactly match clean_company_name() output — the map CSV
    # is generated from the same pipeline via scripts/research_company_sectors.py.
    if SECTOR_MAP_PATH.exists():
        sector_map = pd.read_csv(
            SECTOR_MAP_PATH, usecols=["company_name_clean", "sector"]
        )
        df = df.merge(sector_map, on="company_name_clean", how="left")
    else:
        df["sector"] = pd.NA

    return (
        df[["year", "company_name_raw", "company_name_clean", "issued", "sector"]]
...
if __name__ == "__main__":
    ...
    if "sector" in df.columns and df["sector"].notna().any():
        tagged_companies = df[df["sector"].notna()]["company_name_clean"].nunique()
        total_companies  = df["company_name_clean"].nunique()
        pct = 100.0 * tagged_companies / total_companies
        print(f"  Sector tagged    : {tagged_companies:,} / {total_companies:,} "
              f"unique companies ({pct:.1f}%)")
    else:
